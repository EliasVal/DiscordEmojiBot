...
module.exports = {
    ...
    run: function(client: Discord.Client, args: Array<string>, message: Discord.Message) {
        ...
        // @ts-ignore
        let allowedRoles: Array<string> = global.AllowedRoles ? global.AllowedRoles : []

        switch (args[0]) {
            case "add":
                var targetRole: Discord.Role = message.mentions.roles.first() ? message.mentions.roles.first() : message.guild.roles.cache.has(args[1]) ? message.guild.roles.cache.get(args[1]) : null             
